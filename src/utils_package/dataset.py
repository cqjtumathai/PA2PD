...
class RecDataset(object):
    def __init__(self, config, df=None):
        ...
        for i in check_file_list:
            file_path = os.path.join(self.dataset_path, i)  # 'E:\\学生\\硕士\\我的学生\\熊涛\\对比方法的代码\\MENTOR-main\\data\\baby\\baby.inter'
            if not os.path.isfile(file_path):
                raise ValueError('File {} not exist'.format(file_path))

        # load rating file from data path?
        self.load_inter_graph(config['inter_file_name'])
        self.item_num = int(max(self.df[self.iid_field].values)) + 1  # 7050
        self.user_num = int(max(self.df[self.uid_field].values)) + 1  # 19445

    def load_inter_graph(self, file_name):
        inter_file = os.path.join(self.dataset_path, file_name)  # 'E:\\学生\\硕士\\我的学生\\熊涛\\对比方法的代码\\MENTOR-main\\data\\baby\\baby.inter'
        cols = [self.uid_field, self.iid_field, self.splitting_label]  # ['userID', 'itemID', 'x_label']
    ...
    def copy(self, new_df):
        """Given a new interaction feature, return a new :class:`Dataset` object, whose interaction feature is updated with ``new_df``, and all the other attributes the same.
                Args:
